/**
    ...
    @brief
        Initialization and interrupt handling for the ADC.
...
/**
 * Current ADC DMA buffer.
 */
static int adc_current_buffer = 0;

/**
 * Location in the local ADC DMA buffer
 * to place the next DMA data.
 */
static unsigned int adc_buffer_offset = 0;;
...
void AdcInit() {
    ...
    for (i = 0; i < my_function_block.num_dma_buffers; i++) {
        ...
        result = SDM35541_DMA_Request_Transfer(
            ...
            &adc_dma[adc_buffer_offset],    /* buffer */
            DMA_ADC_BUFFER_SIZE_BYTES,      /* size */
            false,                          /* demand */
            i,                              /* request id */
            0,                              /* timeout */
            false,                          /* wait */
        ...
        adc_buffer_offset += DMA_ADC_BUFFER_SIZE;
    }
...
void AdcHandleInterrupt() {
    ...
    while (adc_filled_buffers[adc_current_buffer]) {
        ...
        if (adc_buffer_offset < ADC_NUM_DATA) {
            result = SDM35541_DMA_Request_Transfer(
                board,
                my_function_block.fb_num,
                ...
                &adc_dma[adc_buffer_offset], // pointer to local buffer
                DMA_ADC_BUFFER_SIZE_BYTES, // size of this buffer in bytes
                false, // no demand
        ...
        /* Stop ADC DMA when all of the data has been received. */
        if (adc_num_ints >= ADC_NUM_INTS) {
            printf("Stopping ADC DMA...\n");
            result = SDM35541_Dma_Stop(board, &my_function_block, SDM35541_DMA_CHANNEL_ADC);
            Fail(result, "Error stopping ADC");
            break;
        }
    }
