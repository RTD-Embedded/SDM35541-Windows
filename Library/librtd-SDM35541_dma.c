/**
	...
	@brief
		SDM35541 DMA library source code


	$Id: librtd-SDM35541_dma.c 105018 2016-12-07 15:47:31Z rgroner $
*/
...
#define SDM35541_DMA_ACTION_LOOPS	10
#define SDM35541_LAST_ACTION_SLEEP_USEC	1000
...
static int
SDM35541_Validate_Dma_Channel_Setup_Args(int dma_channel_direction)
{

	if (dma_channel_direction != SDM35541_DMA_SETUP_DIRECTION_READ &&
	    dma_channel_direction != SDM35541_DMA_SETUP_DIRECTION_WRITE) {
		return -1;
	}
...
static int
SDM35541_Dma_Channel_Get_Last_Action(struct SDM35541_Board_Descriptor *handle,
					...
					uint8_t *last_action)
{
	...
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_LAST_ACTION;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;

	return_code = SDM35541_Read(handle, &ioctl_request);
				...
				unsigned int channel)
{
	...
	ioctl_request.readwrite.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_ACTION;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_ACTION_GO;

	return_code = SDM35541_Write(handle, &ioctl_request);
	 ...
	 */
	return_code = SDM35541_Dma_Channel_Get_Last_Action(handle,
							func_block,
							channel,
							&last_action);

	while ((last_action != ioctl_request.readwrite.access.data.data8) &&
		(loop_count < SDM35541_DMA_ACTION_LOOPS) &&
		(return_code == 0)) {

		SDM35541_Micro_Sleep(SDM35541_LAST_ACTION_SLEEP_USEC);
				...
				unsigned int channel)
{
	...
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_ACTION;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_ACTION_HALT;
				...
				unsigned int channel)
{
	...
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_ACTION;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_ACTION_PAUSE;
...

{
	...
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_ACTION;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_ACTION_CLEAR;
	...
	if (ioctl_request.readwrite.access.data.data8 != SDM35541_DMA_ACTION_CLEAR) {
		ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_ACTION_CLEAR;
		return_code = SDM35541_Write(handle, &ioctl_request);
				...
				uint16_t *read_count)
{
	...
	if (write_count != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_WR_FIFO_CNT;
	...
	if (read_count != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_RD_FIFO_CNT;
				...
				enum SDM35541_Fifo_States *state)
{
	...
	*state = SDM35541_FIFO_UNKNOWN;

	return_code = SDM35541_Dma_Get_Fifo_Counts(handle,
							func_block,
							channel,
							&write_count,
							&read_count);
	...
	if (read_count & 0x8000) {
		*state = SDM35541_FIFO_EMPTY;
	}
	else if (write_count & 0x8000) {
		*state = SDM35541_FIFO_FULL;
	}
	else {
		*state = SDM35541_FIFO_HAS_DATA;
	}
				...
				int error_enable)
{
	...
	ioctl_request.modify.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_SETUP;
	ioctl_request.modify.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.modify.access.size = SDM35541_PCI_REGION_ACCESS_8;

	if (enable) {
		val8b = SDM35541_DMA_SETUP_INT_ENABLE;
	}
	if (error_enable) {
		val8b |= SDM35541_DMA_SETUP_ERR_INT_ENABLE;
	}

	mask8b = SDM35541_DMA_SETUP_INT_MASK;
	mask8b |= SDM35541_DMA_SETUP_ERR_INT_MASK;
				...
				int ignore_used)
{
	...
	return_code = SDM35541_Validate_Dma_Channel_Setup_Args(direction);
	...
	if (ignore_used) {
		val |= SDM35541_DMA_SETUP_IGNORE_USED;
	...
	ioctl_request.modify.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.modify.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.modify.mask.mask8 = SDM35541_DMA_SETUP_IGNORE_USED_MASK |
					  SDM35541_DMA_SETUP_DIRECTION_MASK;
				...
				int direction)
{
	...
	ioctl_request.modify.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.modify.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.modify.mask.mask8 = SDM35541_DMA_SETUP_DIRECTION_MASK;
	ioctl_request.modify.access.data.data8 = val;
	ioctl_request.modify.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_SETUP;
				...
				int *stat_invalid)
{
	...
	if (stat_overflow != NULL || stat_underflow != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset + SDM35541_OFFSET_DMA_STAT_OVERFLOW;
		ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_16;
	...
	if (stat_used != NULL || stat_invalid != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset + SDM35541_OFFSET_DMA_STAT_USED;
		ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_16;
				...
				int *stat_complete)
{
	...
	if (current_buffer != NULL || current_count != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset + SDM35541_OFFSET_DMA_CURRENT_COUNT;
	...
	if (current_action != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset + SDM35541_OFFSET_DMA_ACTION;
		ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	...
	if (stat_overflow != NULL || stat_underflow != NULL || stat_used != NULL || stat_invalid != NULL) {
        return_code = SDM35541_Dma_Get_Errors(handle,
                            func_block,
                            channel,
                            stat_overflow,
                            stat_underflow,
	...
	if (stat_complete != NULL) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset + SDM35541_OFFSET_DMA_STAT_COMPLETE;
		ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
					...
					uint32_t *current_count)
{
	...
	ioctl_request.readwrite.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
						SDM35541_OFFSET_DMA_CURRENT_COUNT;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_32;
				...
				int *has_error)
{
	...
	*has_error = 0;
	...
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_16;
	ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_STAT_OVERFLOW;
				...
				uint8_t ctrl)
{
	...
	ioctl_request.readwrite.access.offset =
			...
			SDM35541_OFFSET_DMA_BUFFER_CTRL;
				...
				uint32_t *size)
{
	...
	ioctl_request.readwrite.access.offset =
			func_block->dma_channel[channel].buffer_start_offset[buffer] +
			SDM35541_OFFSET_DMA_BUFFER_STAT;
	...
	*control = ioctl_request.readwrite.access.data.data8;
	...
	ioctl_request.readwrite.access.offset =
			func_block->dma_channel[channel].buffer_start_offset[buffer] +
			SDM35541_OFFSET_DMA_BUFFER_SIZE;

	return_code = SDM35541_Read(handle, &ioctl_request);

	*size = SDM35541_BIT_MASK_DMA_BUFFER_SIZE & ioctl_request.readwrite.access.data.data32;
				...
				int *is_used)
{
	...
	*is_used = 0;
	...
	if (ioctl_request.readwrite.access.data.data8 & SDM35541_DMA_BUFFER_STATUS_USED_MASK) {
		*is_used = 1;
	}
	...
	*channel_error = 0;
	...
	do {
		...
		if (stat_used_inv_comp & 0x00FF) {
            ...
            ignore_used = ioctl_request.readwrite.access.data.data8 & SDM35541_DMA_SETUP_IGNORE_USED;
					...
					int clear_complete)
{
	...
	ioctl_request.readwrite.access.region = SDM35541_PCI_REGION_FB;
	ioctl_request.readwrite.access.size = SDM35541_PCI_REGION_ACCESS_8;
	ioctl_request.readwrite.access.data.data8 = SDM35541_DMA_STATUS_CLEAR;

	if (clear_overflow) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_STAT_OVERFLOW;

		result = SDM35541_Write(handle, &ioctl_request);
	...
	if (clear_underflow) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_STAT_UNDERFLOW;
	...
	if (clear_invalid) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_STAT_INVALID;
	...
	if (clear_complete) {
		ioctl_request.readwrite.access.offset = func_block->dma_channel[channel].control_offset +
							SDM35541_OFFSET_DMA_STAT_COMPLETE;
					...
					uint32_t *buffer_size)
{
	...
	*buffer_size = (ioctl_request.readwrite.access.data.data32 & SDM35541_BIT_MASK_DMA_BUFFER_SIZE);
				...
				uint32_t buffer_size)
{
	...
	if ((buffer_size & 0x3) || (buffer_size > SDM35541_BIT_MASK_DMA_BUFFER_SIZE)) {
		errno = EINVAL;
		return -1;
	}
	...
	ioctl_request.modify.mask.mask32 = SDM35541_BIT_MASK_DMA_BUFFER_SIZE;
